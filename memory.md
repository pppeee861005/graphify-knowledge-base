...
## 🎯 核心工作導航（2026年6月11日 — 當前主要工作）

> **我們當前的兩個最核心的戰略項目，指導整個知識體系的發展方向。**

### 📍 1. 信任生產線 SOP（四件套文體的生產規程）

**文件位置**：`出版管理/策略分析/信任生產線-SOP.md`

**定位**：四件套文體（⚡戰報 → 🏭機房 → 📚教科書 → 🎭劇場）的開發流程標準作業程序
...
回鏈原則：每層往源頭掛連結，信息只能順流不能逆流
```

**適用範圍**：
- AI 指揮官 Substack 全部內容生產
- 所有開源專案的配套發布（租約哨兵、補助哨兵、顧問團等）
- 計劃系列（計劃1-13）的報告體系

**與記憶系統的關係**：
- 戰報、機房、教科書的寫作都會生成 work_log_YYYYMMDD.md
- 思想進程（Workflow中的發現）優先發在戰報層
- Memory記錄的是「寫作過程的決策」，而信任生產線SOP規範的是「發布內容的生成規程」

---

### 📍 2. 產品線計劃（AI顧問團的18個月戰略）

**文件位置**：`出版管理/策略分析/產品線計劃.md`

**定位**：8個席位的開源公共設施，將「顧問民主化」從單一場景推廣到完整產品線

**核心架構**：

| 席位 | 場景 | 優先級 | 狀態 |
|------|------|--------|------|
| 1️⃣ 合約顧問 | 租約/勞動契約/買賣契約 | 🔴 P0 | ✅ 進行中（計劃13租約哨兵） |
| 2️⃣ 行政官僚顧問 | 補助/報稅/社福權益 | 🔴 P0 | ⏳ 規劃中（補助哨兵） |
| 3️⃣ 財務顧問 | 貸款/保單/金融商品 | 🟡 P1 | 🟢 待開始 |
| 4️⃣ 大宗購買顧問 | 買房/買車/裝修 | 🟡 P1 | 🟢 待開始 |
| 5️⃣ 健康顧問 | 症狀/醫囑/疫苗 | 🟡 P1 | 🟢 待開始 |
| 6️⃣ 職涯顧問 | 履歷/offer/產業預警 | 🟢 P2 | 🟢 待開始 |
| 7️⃣ 教育顧問 | 升學/補習/代際傳遞 | 🟢 P2 | 🟢 待開始 |
| 8️⃣ 資訊心理顧問 | 查證/詐騙識別/決策干預 | 🟢 P2 | 🟢 待開始 |

**18個月分階段計劃**：
- **Phase 1**（2026-06～09，4個月）：完成席位1-2 + 方法論文檔化
- **Phase 2**（2026-10～2027-03，6個月）：完成席位3-5 + MVP評測
- **Phase 3**（2027-04～12，9個月）：完成席位6-8 + 統一平台 + 國家本地化

**進度追蹤**：`出版管理/策略分析/進度追蹤表.md`（每週五更新）

**與信任生產線的連接**：
- 每個席位都遵循「戰報→機房→教科書」的三層報告體系
- 計劃13租約哨兵是第一個完整的範例
- 後續席位2-8都會複用同一套四件套發布規程
...
```
信任生產線 SOP（HOW to publish）
    ↓
    告訴我們「怎麼把成果發佈出去」
    （戰報潦草 → 機房審計 → 教科書複用 → 劇場傳播）
...
### 🏗️ 人機協作層級架構（計劃13_租約哨兵的具體應用）

**這個框架解釋了在「規則庫比對型 Workflow」中，不同層級如何協作。**

| 層級 | 協作角色 | 生活比喻 | 租約哨兵中的應用 |
|------|---------|---------|-----------------|
| **L0** | 人類親執（新增） | 人類，親自動手 | 主廚親自挑食材、試菜定味 | 通讀憲法、嚴重度定稿、修憲審批、漏報覆盤定性 |
...
**L0 - 人類親執（不可替代的決策）**
- 憲法級別的定義只能由人做
- 「嚴重度」判定涉及法律和倫理判斷，AI 只能提案，人必須簽名
- 漏報的根因分析和修法決策必須有人的價值判斷

**L1 - Subagents（AI 輔助的結構化工作）**
- Claude 幫助起草規則庫的結構化版本
- Claude 翻譯複雜條款為租客能理解的白話
- Claude 執行 OCR 等單一、明確的 AI 任務

**L1.5 - Agent View（人的中央監控）**
- 人類視角監看多個 Agent 並行工作的進度
- 決定什麼時候叫停、什麼時候加速、什麼時候需要人工介入
- 確保多個 Agent 的工作不會產生矛盾

**L2 - Agent Teams（相互檢查的協作）**
- 第二位標注者驗證第一位的工作（對抗性驗證）
- 紅隊 Agent 專門尋找系統可能遺漏的風險
- 多個 Agent 的「互相審查」確保品質

**L3 - Dynamic Workflows（自動化的流程）**
- 五階段審查管線自動運轉（OCR → 切分 → 規則比對 → 風險解釋 → 信心評分）
- 評測 harness 自動計算漏報率、誤報率
- 規則哨兵 Agent 自動檢測官方規範更新

#### 核心設計原則

```
最終保障：L0（人的不可替代性）
具體執行：L1–L3（AI 和 Agent 的協作）
...
| 文件/項目 | 狀態 | 完成度 | 下一步 |
|----------|------|--------|--------|
| **信任生產線 SOP** | ✅ 完成 | 100% | 在計劃系列中逐項應用 |
| **產品線計劃** | ✅ 完成 | 100% | 啟動Phase 1執行 |
| **進度追蹤表** | ✅ 完成 | 100% | 每週五更新監控 |
| **計劃13租約哨兵** | ⏳ 進行中 | 50% | W2核心Pipeline開發 |
| **計劃13補助哨兵** | 🟢 規劃中 | 0% | 啟動定義書設計 |
...
| 任務 | 第一步 | 參考文件 |
|------|--------|----------|
| **寫大綱** | 先讀爆款文章編寫規範 | `出版管理/爆款文章編寫規範_AI工程派詩人.md` |
...
### 2026年6月10日 - 《新的物種》大綱+正文完成 + 爆款規範驗證
- **文件**：`memory/work_log_20260610.md`

#### 核心成果
- ✅ 《新的物種：人類的數位新夥伴》7層完整大綱（參考爆款規範）
  - 3個標題候選、7個獨立金句、4個領域分工表
  - 核心轉折：「利益衝突論」打破恐懼敘事
  - 哲學昇華：「我思故我在」→「我們共生故我們在」

- ✅ 正文編寫完成（1,450字，黃金長度）
  - 開場（150字）→ 六大特徵（350字）→ 轉折（150字）→ 論證（300字）→ 昇華（200字）→ 展望（150字）→ 結尾（100字）
  - 黃金配方 7/7 達成（40%知識+15%感性+15%比喻+10%鉤子+10%細節+5%雞湯+5%引流）

- ✅ 工作流程記錄到 memory.md
  - 新增「✍️ 寫作流程提示」表：寫大綱先讀爆款規範

#### 工作耗時
- 3.5 小時（含大綱 + 正文 + 日誌）

#### 核心創新
- 用逻輯質疑（利益衝突）而非感性否定，打破「AI威脅」敘事
- 森林共生類比系統：菌根網絡 → 眼睛耳朵 → 輪子與雙腿
- AI工程派詩人的完整體現：理性表格+感性比喻+哲學金句

#### 參考文件
- 📄 [大綱檔](讀書筆記/哈拉瑞論人工智慧/新的物種草稿大綱.md)
- 📄 [正文檔](讀書筆記/哈拉瑞論人工智慧/新的物種_正文.md)
- 📄 [完整日誌](memory/work_log_20260610.md)

---

### 2026年6月8日 - 核心理論#3 精妙分割 + 2週發布策略完成
- **文件**：`日誌/work_log_20260608.md`

#### 核心成果
- ✅ 《核心理論#3》從8500字完整重寫，分成「上篇診斷與覺醒」+「下篇修行與蛻變」兩篇精品
  - 上篇（4,500字）：四困故事→破局原則→每日思考→預告下篇
  - 下篇（4,500字）：三個修行→每日檢視→李明蛻變→行動呼籲
- ✅ AI產品示例現代化升級（替換稻盛和夫案例為當代AI創業故事）
  - 新示例：融資焦慮→停止看新聞→聚焦用戶→65%→82%
- ✅ 2週發布策略完整設計
  - 第1週上篇：診斷問題，激發改變慾望，預告下篇（製造懸念）
  - 第2週下篇：提供方法，立即行動，完成認知→動機→方法→行動的完整漏斗

#### 關鍵創新
- 敘事弧設計：讀者在兩週內經歷完整的思想蛻變
- 懸念製造：上篇預告激發讀者自動期待下篇
- 示例升級：更現代、更邏輯清晰、更親切的職場案例

#### 工作耗時
- 5.5 小時（包括重寫、分割、示例升級、策略設計）
